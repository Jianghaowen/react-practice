...
router.post('/login', cors.corsWithOptions, (req, res, next) => {
  ...
  passport.authenticate('local' , (err, user , info) => {
    ...
    if(!user) {
      res.statusCode = 401;
      res.setHeader('Content-Type', 'application/json');
      return res.json({success: false, status: 'the account can not found' , err : info});
    }
    req.logIn(user , (err) => {
      if (err) {
        res.statusCode = 401;
        res.setHeader('Content-Type', 'application/json');
        return res.json({success: false, status: 'login unsuccessful' , err : 'Could not be login user'});
      }
